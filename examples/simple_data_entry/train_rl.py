...
def main(
    ...
    train_dataloader = DataLoader(train_ds, batch_size=1, collate_fn=lambda x: x[0], num_workers=0)
    test_dataloader = DataLoader(test_ds, batch_size=1, collate_fn=lambda x: x[0], num_workers=0)
    ...
    for epoch in range(10):
        ...
        for batch in progress:
            ...
            if global_step > 0 and global_step % eval_checkpoint_steps == 0:
            ...
            with accelerator.accumulate(model):
                ...
                # TODO: replace loss with advantage-weighted loss
                # 1. Compute per sample advantage based on reward
                # 2. Apply advantage per token loss if possible
                # 3. otherwise, compute loss per token given the logits and weight manually
                # by advantage

                # TODO: Since we just use batch size 1, we can simply scale the loss
                # directly by the advantage here for now?
                loss = outputs.loss * batch["reward"]
