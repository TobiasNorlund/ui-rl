...
async def run_cua_session(
    ...
    try:
        log("Starting...")
        await _await_ready(cluster_host, session_id, session_timeout)
        log("Ready")
        ...
        state = await _act(cluster_host, session_id, action)
        ...
        # Compute reward
        progress = await _get_progress(cluster_host, session_id)
...
async def _await_ready(cluster_host: str, session_id: str, session_timeout: int):
...
async def _act(cluster_host: str, session_id: str, action: Action) -> State:
    """
                ...
                async with session.get(
                    ...
                    if resp.status >= 500:
                        if attempt < 2:  # Not the last attempt
                            log(f"Server error {resp.status}, retrying... (attempt {attempt + 1}/3)")
                            continue
                        else:
...
async def _get_progress(cluster_host, session_id) -> Dict:
    """
